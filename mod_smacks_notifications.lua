...
function handle_smacks_message(event)
	...
	if session.regid ~= nil then
		...
		if GCM_API_KEY != nil then
			...
			http.request(gcm_uri, {headers = {name = "Authorization", value = GCM_API_KEY}, body = http.formencode({name = "data.message", value = message_body, name = "recipients_ids", value = session.regid, name = "collapse_key", value ="New messages"})}, function(response, code, request) 
